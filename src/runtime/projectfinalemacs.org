...
* Visualisation
   Goal is to analyze the information about the different repositories of Virtual Labs in Git hub
** Serve template
#+NAME: serve_template
#+BEGIN_SRC python
...
@app.route("/d3lang")
def d3lang():
    repoLangDataAsJSON14 = json.dumps(getRepoLanguageDataAsJSON(2014))
    repoLangDataAsJSON15 = json.dumps(getRepoLanguageDataAsJSON(2015))
    repoLangDataAsJSON16 = json.dumps(getRepoLanguageDataAsJSON(2016))
    repoLangDataAsJSON17 = json.dumps(getRepoLanguageDataAsJSON(2017))
    print("repoLangDataAsJSON : " , repoLangDataAsJSON14)
...
var makeChart = function(dataset, chart){
...
.chart-container{
  ...
  <body>
    <table>
      <tr>
        <td>
        <h1 text-align="center">Technologies till 2014</h1>
        <div id="chart14"  style="width: 900px; height: 900px;" class="chart-container"></div>
        <script type="text/javascript">
          var dataset14 = {{repoLangDataAsJSON14 | safe}};
          makeChart(dataset14, "chart14");
        </script>
      </td>
      <td>
        <h1 text-align="center">Technologies till 2015</h1>
        <div id="chart15"  style="width: 900px; height: 900px;" class="chart-container"></div>
        <script type="text/javascript">
          var dataset15 = {{repoLangDataAsJSON15 | safe}};
          makeChart(dataset15, "chart15");
        </script>
      </td>
    </tr>
    <tr>
      <td>
        <h1 text-align="center">Technologies till 2016</h1>
        <div id="chart16"  style="width: 900px; height: 900px;" class="chart-container"></div>
        <script type="text/javascript">
          var dataset16 = {{repoLangDataAsJSON16 | safe}};
          makeChart(dataset16, "chart16");
        </script>
      </td>
      <td>
        <h1>Technologies till 2017</h1>
        <div id="chart17"  style="width: 900px; height: 900px;" class="chart-container"></div>
        <script type="text/javascript">
          var dataset17 = {{repoLangDataAsJSON17 | safe}};
          makeChart(dataset17, "chart17");
        </script>
      </td>
    </tr>
     <a href="/topclosedissues">Go to Top Repostories</a> 
  </body>
...
<div id="barchart"  style="width: 400px; height: 50px;" ></div>
    <script>
        restOfTheData();
    </script>
</body>
...
#+BEGIN_SRC html :tangle templates/groupclosedissue.html
...
<head>
<h1 align="center" style="color:black;">Repositories with highest number of closed issues</h1>
</head>
<body>
...
#+BEGIN_SRC python
def getRepoLanguageDataAsJSON(year):
    repoLangData2 = []
    dict = {}
    #data = getdata()
    for item in data:
        intlocalyear = getYearofRepo(item)
        currYear = getCurYear()
        if intlocalyear <= year: #considering all repositories
            if item['language'] is None:
...
def getRepoCommitDevCIssueDataAsJSON():
    ...
    for item in data:
        ...
        #if count <7:
            #count += 1
        contr = item['contributors_url']
        contributors_url = str(contr)
...
temparr = []
for i in jsondata:
    if i['closedIssueCount'] > 0:
        temparr.insert(k,jsondata[k])
        k += 1

temparr = sorted(temparr, key=lambda x: x['closedIssueCount'], reverse=True)
...
from repository import commitsort, closedissuesort, devsort, commitar, devar, closedissuear, temparr
...
** sources
#+BEGIN_SRC python :tangle virtualabs.py :eval no :noweb yes
<<imports_for_sources>>
<<serve_template>>
<<ind>>
<<d3lang>>
